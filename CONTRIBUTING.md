...
[fork]: https://github.com/sudeeshjohn/go-projects/fork
...
If you open a pull request you must be able to clearly explain what your changes do and how they alter the behaviour of the SDK without relying upon AI tools or prompting to roundtrip the reviewer's questions.
...
Because these SDKs are in production use by many people and applications, we code review all additions.
...
- Make sure your code is supported by all the Go versions we support (Go 1.23.0+).
...
- Add example programs in the `examples/` directory demonstrating new functionality.
...
- Write a [good commit message](http://tbaggery.com/2008/04/19/a-note-about-git-commit-messages.html). Use conventional commit format: `type(scope): description`
  - Types: `feat`, `fix`, `docs`, `style`, `refactor`, `test`, `chore`
  - Scopes: `phmc`, `svc`, `hmcctl`, `infractl`, `pvcctl`, etc.
...
### For infra-go-sdk (phmc/svc)

- Follow the existing API patterns and naming conventions
- Use `context.Context` as the first parameter for all API calls
- Return errors with proper context using `fmt.Errorf` with `%w`
...
### For CLI tools (hmcctl/infractl/pvcctl)
...
Integration tests require access to real HMC or SVC systems. Mark integration tests with build tags:
